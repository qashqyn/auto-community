...
const Paginate = ({tags, page, type, filter, searchText, category, suits, condition}) => {
    ...
    useEffect(() => {
        if(page){
            if(type === 'news')
                ...
                dispatch(getMarketPosts(page, searchText, category, suits, condition));
                
        }
    },[dispatch, tags, page, type, suits, category, condition]);
